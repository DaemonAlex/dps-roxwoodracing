...
local function fetchTracks()
  if tracks then return tracks end
  local all = fetchLines()
  if not all then return nil end
  tracks = Practice.Cluster(all, Config.Practice.trackJoinRadius)
  return tracks
end
...
local function spawn()
  ...
  local all = curTrack and curTrack.lines
...
startSteering = function()
  CreateThread(function()
    ...
    while running do
      ...
      local main = curTrack and curTrack.lines[1]
      if main then
        local n = #main.pts
        local seen = {}
        for _, pid in ipairs(GetActivePlayers()) do
...
local function armTracks()
  ...
  for _, t in ipairs(ts) do
    ...
    local s0 = t.lines[1].pts[1]
    startZones[#startZones + 1] = lib.zones.sphere({
      coords = vector3(s0.x, s0.y, s0.z),
      radius = mode.startRadius or 15.0,
      onEnter = function()
        if curTrack ~= t or IsRaceActive() or not allowed() then return end
  ...
  log(('%d track(s) armed: %s'):format(#ts, table.concat((function() local n = {} for _, t in ipairs(ts) do n[#n + 1] = ('%s (%d lines, r %.0f m)'):format(t.id, #t.lines, t.radius) end return n end)(), ', ')))
  return true
end
...
CreateThread(function()
  ...
  Wait(2000)
  if not armTracks() then log('no line stored; record one with /recordline') end
end)
...
RegisterNetEvent('dps-roxwoodracing:practice:host', function(on, track)
  isHost = on == true and (track == nil or (curTrack ~= nil and curTrack.id == track))
  log(('host=%s track=%s'):format(tostring(isHost), tostring(track)))
  if isHost then
...
RegisterNetEvent('dps-roxwoodracing:line:saved', function(ok)
  ...
  despawn()
  lines, tracks = nil, nil
  if armTracks() and nearTrack then CreateThread(spawn) end
end)

-- /practicestatus: one F8 line per car (where it is, how fast, laps, stuck resets)
RegisterCommand('practicestatus', function()
  log(('running=%s nearTrack=%s track=%s host=%s cars=%d allowed=%s pace=%.2f target=%s clock=%s'):format(
    tostring(running), tostring(nearTrack), curTrack and curTrack.id or 'none', tostring(isHost), #cars, tostring(allowed()), paceMult,
